...
#include <fcntl.h>
    ...
    #include <orbisGl2par_shapes.h>    // Shapes 3d parametric generation
#endif
//----------------------------------------------------------------------------------
// Defines and Macros
//----------------------------------------------------------------------------------
...
Model LoadModel(const char *fileName)
{
...
#if defined(SUPPORT_MESH_GENERATION)
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] No meshes can be loaded, default to cube mesh\n",__FUNCTION__,fileName);
		model.meshes[0] = GenMeshCube(1.0f, 1.0f, 1.0f);
#else
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] No meshes can be loaded, and can't create a default mesh. The raylib mesh generation is not supported (SUPPORT_MESH_GENERATION).\n",__FUNCTION__ ,fileName);
#endif
	}
	else
	{
	...
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] No materials can be loaded, default to white material\n",__FUNCTION__ ,fileName);
...
void UnloadModel(Model model)
{
	...
	free(model.bones);
	free(model.bindPose);
	debugNetPrintf(INFO, "[ORBISGL] %s Unloaded model data from RAM and VRAM\n",__FUNCTION__);
}
...
Material *LoadMaterials(const char *fileName, int *materialCount)
{
	...
	if (IsFileExtension(fileName, ".mtl"))
	{
		...
		if (result != TINYOBJ_SUCCESS) {
			debugNetPrintf(ERROR, "[ORBISGL] %s [%s] Could not parse Materials file\n",__FUNCTION__,fileName);
		}
		// TODO: Process materials to return
		tinyobj_materials_free(mats, count);
	}
#else
	debugNetPrintf(ERROR, "[ORBISGL] %s [%s] Materials file not supported",__FUNCTION__,fileName);
#endif
...
{
	if (meshId >= model->meshCount) debugNetPrintf(ERROR, "[ORBISGL] %s Mesh id greater than mesh count\n",__FUNCTION__);
	else if (materialId >= model->materialCount) debugNetPrintf(ERROR,"[ORBISGL] %s Material id greater than material count\n",__FUNCTION__);
...
ModelAnimation *LoadModelAnimations(const char *filename, int *animCount)
{
	...
	int iqmFile = -1;
	IQMHeader iqm;
	iqmFile = orbisNfsOpen(filename,O_RDONLY,0);
	if (iqmFile<0)
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] Unable to open file\n",__FUNCTION__,filename);
		return NULL;
	}
	// Read IQM header
	orbisNfsRead(iqmFile,&iqm, sizeof(IQMHeader));
	if (strncmp(iqm.magic, IQM_MAGIC, sizeof(IQM_MAGIC)))
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s Magic Number \"%s\"does not match.\n",__FUNCTION__,iqm.magic);
		orbisNfsClose(iqmFile);
		return NULL;
	}
	if (iqm.version != IQM_VERSION)
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s IQM version %i is incorrect.\n",__FUNCTION__,iqm.version);
		orbisNfsClose(iqmFile);
		return NULL;
	}
	// Get bones data
	IQMPose *poses = malloc(iqm.num_poses*sizeof(IQMPose));
	orbisNfsLseek(iqmFile, iqm.ofs_poses, SEEK_SET);
	orbisNfsRead(iqmFile,poses, iqm.num_poses*sizeof(IQMPose));
	// Get animations data
	*animCount = iqm.num_anims;
	IQMAnim *anim = malloc(iqm.num_anims*sizeof(IQMAnim));
	orbisNfsLseek(iqmFile, iqm.ofs_anims, SEEK_SET);
	orbisNfsRead(iqmFile,anim, iqm.num_anims*sizeof(IQMAnim));
	ModelAnimation *animations = malloc(iqm.num_anims*sizeof(ModelAnimation));
	// frameposes
	unsigned short *framedata = malloc(iqm.num_frames*iqm.num_framechannels*sizeof(unsigned short));
	orbisNfsLseek(iqmFile, iqm.ofs_frames, SEEK_SET);
	orbisNfsRead(iqmFile,framedata, iqm.num_frames*iqm.num_framechannels*sizeof(unsigned short));
	orbisNfsClose(iqmFile);
	...
	for (int a = 0; a < iqm.num_anims; a++)
	{
		...
		animations[a].bones = malloc(iqm.num_poses*sizeof(BoneInfo));
		animations[a].framePoses = malloc(anim[a].num_frames*sizeof(Transform *));
		...
		for (int j = 0; j < anim[a].num_frames; j++) animations[a].framePoses[j] = malloc(iqm.num_poses*sizeof(Transform));
...
{
	Mesh mesh = { 0 };
	mesh.vboId = (unsigned int *)calloc(MAX_MESH_VBO, sizeof(unsigned int));
...
void MeshTangents(Mesh *mesh)
{
	...
	else debugNetPrintf(ERROR, "[ORBISGL] %s Mesh tangents already exist\n",__FUNCTION__);
	...
	debugNetPrintf(INFO, "[ORBISGL] %s Tangents computed for mesh\n",__FUNCTION__);
}
...
static Model LoadOBJ(const char *fileName)
{
	...
	char *data = NULL;
	// Load model data
	int objFile = orbisNfsOpen(fileName, O_RDONLY,0);
	if (objFile >= 0)
	{
		long length=orbisNfsLseek(objFile, 0, SEEK_END);
		orbisNfsLseek(objFile, 0, SEEK_SET);    // Reset file pointer
		data = (char *)malloc(length);
		orbisNfsRead(objFile,data, length);
		dataLength = length;
		orbisNfsClose(objFile);
	}
	if (data != NULL)
	{
		unsigned int flags = TINYOBJ_FLAG_TRIANGULATE;
		int ret = tinyobj_parse_obj(&attrib, &meshes, &meshCount, &materials, &materialCount, data, dataLength, flags);
		if (ret != TINYOBJ_SUCCESS) debugNetPrintf(ERROR, "[ORBISGL] %s [%s] Model data could not be loaded\n",__FUNCTION__,fileName);
		else debugNetPrintf(INFO, "[ORBISGL] %s [%s] Model data loaded successfully: %i meshes / %i materials\n",__FUNCTION__,fileName, meshCount, materialCount);
	...
	debugNetPrintf(INFO, "[ORBISGL] %s [%s] Model loaded successfully in RAM (CPU)\n",__FUNCTION__,fileName);
	return model;
}
#endif
...
static Model LoadIQM(const char *fileName)
{
	...
	*/
	...
	if (iqmFile < 0)
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] IQM file could not be opened\n",__FUNCTION__,fileName);
		return model;
	}
	orbisNfsRead(iqmFile,&iqm,sizeof(IQMHeader));  // Read IQM header
	if (strncmp(iqm.magic, IQM_MAGIC, sizeof(IQM_MAGIC)))
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] IQM file does not seem to be valid\n",__FUNCTION__,fileName);
		orbisNfsClose(iqmFile);
		return model;
	}
	if (iqm.version != IQM_VERSION)
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] IQM file version is not supported (%i).\n",__FUNCTION__,fileName, iqm.version);
		orbisNfsClose(iqmFile);
		return model;
	}
	// Meshes data processing
	imesh = malloc(sizeof(IQMMesh)*iqm.num_meshes);
	orbisNfsLseek(iqmFile, iqm.ofs_meshes, SEEK_SET);
	orbisNfsRead(iqmFile,imesh, sizeof(IQMMesh)*iqm.num_meshes);
	model.meshCount = iqm.num_meshes;
	model.meshes = calloc(model.meshCount, sizeof(Mesh));
	char name[MESH_NAME_LENGTH] = { 0 };
	for (int i = 0; i < model.meshCount; i++)
	{
		orbisNfsLseek(iqmFile, iqm.ofs_text + imesh[i].name, SEEK_SET);
		orbisNfsRead(iqmFile,name, sizeof(char)*MESH_NAME_LENGTH);     // Mesh name not used...
		model.meshes[i].vertexCount = imesh[i].num_vertexes;
		model.meshes[i].vertices = calloc(model.meshes[i].vertexCount*3, sizeof(float));       // Default vertex positions
		model.meshes[i].normals = calloc(model.meshes[i].vertexCount*3, sizeof(float));        // Default vertex normals
		model.meshes[i].texcoords = calloc(model.meshes[i].vertexCount*2, sizeof(float));      // Default vertex texcoords
		model.meshes[i].boneIds = calloc(model.meshes[i].vertexCount*4, sizeof(float));        // Up-to 4 bones supported!
		model.meshes[i].boneWeights = calloc(model.meshes[i].vertexCount*4, sizeof(float));    // Up-to 4 bones supported!
		model.meshes[i].triangleCount = imesh[i].num_triangles;
		model.meshes[i].indices = calloc(model.meshes[i].triangleCount*3, sizeof(unsigned short));
		// Animated verted data, what we actually process for rendering
		// NOTE: Animated vertex should be re-uploaded to GPU (if not using GPU skinning)
		model.meshes[i].animVertices = calloc(model.meshes[i].vertexCount*3, sizeof(float));
		model.meshes[i].animNormals = calloc(model.meshes[i].vertexCount*3, sizeof(float));
		model.meshes[i].vboId = (unsigned int *)calloc(MAX_MESH_VBO, sizeof(unsigned int));
	}
	// Triangles data processing
	tri = malloc(iqm.num_triangles*sizeof(IQMTriangle));
	orbisNfsLseek(iqmFile, iqm.ofs_triangles, SEEK_SET);
	orbisNfsRead(iqmFile,tri, iqm.num_triangles*sizeof(IQMTriangle));
	for (int m = 0; m < model.meshCount; m++)
	{
		int tcounter = 0;
	...
	va = malloc(iqm.num_vertexarrays*sizeof(IQMVertexArray));
	orbisNfsLseek(iqmFile, iqm.ofs_vertexarrays, SEEK_SET);
	orbisNfsRead(iqmFile,va, iqm.num_vertexarrays*sizeof(IQMVertexArray));
	for (int i = 0; i < iqm.num_vertexarrays; i++)
	{
		switch (va[i].type)
		{
			case IQM_POSITION:
			{
				vertex = malloc(iqm.num_vertexes*3*sizeof(float));
				orbisNfsLseek(iqmFile, va[i].offset, SEEK_SET);
				orbisNfsRead(iqmFile,vertex, iqm.num_vertexes*3*sizeof(float));
			...
			case IQM_NORMAL:
			{
				normal = malloc(iqm.num_vertexes*3*sizeof(float));
				orbisNfsLseek(iqmFile, va[i].offset, SEEK_SET);
				orbisNfsRead(iqmFile,normal, iqm.num_vertexes*3*sizeof(float));
				for (int m = 0; m < iqm.num_meshes; m++)
				{
					int vCounter = 0;
					for (int i = imesh[m].first_vertex*3; i < (imesh[m].first_vertex + imesh[m].num_vertexes)*3; i++)
			...
			case IQM_TEXCOORD:
			{
				text = malloc(iqm.num_vertexes*2*sizeof(float));
				orbisNfsLseek(iqmFile, va[i].offset, SEEK_SET);
				orbisNfsRead(iqmFile,text, iqm.num_vertexes*2*sizeof(float));
				for (int m = 0; m < iqm.num_meshes; m++)
				{
					int vCounter = 0;
			...
			{
				blendi = malloc(iqm.num_vertexes*4*sizeof(char));
				orbisNfsLseek(iqmFile, va[i].offset, SEEK_SET);
				orbisNfsRead(iqmFile,blendi, iqm.num_vertexes*4*sizeof(char));
				for (int m = 0; m < iqm.num_meshes; m++)
				{
					int boneCounter = 0;
			...
			{
				blendw = malloc(iqm.num_vertexes*4*sizeof(unsigned char));
				orbisNfsLseek(iqmFile, va[i].offset, SEEK_SET);
				orbisNfsRead(iqmFile,blendw, iqm.num_vertexes*4*sizeof(unsigned char));
				for (int m = 0; m < iqm.num_meshes; m++)
				{
					int boneCounter = 0;
					for (int i = imesh[m].first_vertex*4; i < (imesh[m].first_vertex + imesh[m].num_vertexes)*4; i++)
	...
	ijoint = malloc(iqm.num_joints*sizeof(IQMJoint));
	orbisNfsLseek(iqmFile, iqm.ofs_joints, SEEK_SET);
	orbisNfsRead(iqmFile,ijoint, iqm.num_joints*sizeof(IQMJoint));
	model.boneCount = iqm.num_joints;
	model.bones = malloc(iqm.num_joints*sizeof(BoneInfo));
	model.bindPose = malloc(iqm.num_joints*sizeof(Transform));
	for (int i = 0; i < iqm.num_joints; i++)
	{
		// Bones
		model.bones[i].parent = ijoint[i].parent;
		orbisNfsLseek(iqmFile, iqm.ofs_text + ijoint[i].name, SEEK_SET);
		orbisNfsRead(iqmFile,model.bones[i].name, BONE_NAME_LENGTH*sizeof(char));
		// Bind pose (base pose)
...
static Image LoadImageFromCgltfImage(cgltf_image *image, const char *texPath, Color tint)
{
    ...
    if (image->uri)
    {
            ...
            (image->uri[4] == ':'))
        {
            ...
            if (image->uri[i] == 0) debugNetPrintf(ERROR, "[ORBISGL] %s CGLTF Image: Invalid data URI\n",__FUNCTION__);
            else
            {
                int size;
    ...
    {
        unsigned char *data = malloc(image->buffer_view->size);
...
static Model LoadGLTF(const char *fileName)
{
	...
	*************************************************************************************/
	...
	Model model = { 0 };
	// glTF file loading
	int gltfFile = orbisNfsOpen(fileName, O_RDONLY,0);
	if (gltfFile < 0)
	{
		debugNetPrintf(ERROR, "[ORBISGL] %s [%s] glTF file could not be opened\n",__FUNCTION__,fileName);
		return model;
	}
	size=orbisNfsLseek(gltfFile, 0, SEEK_END);
	orbisNfsLseek(gltfFile, 0, SEEK_SET);
	void *buffer = malloc(size);
	orbisNfsRead(gltfFile,buffer, size);
	orbisNfsClose(gltfFile);
	// glTF data loading
	...
	{
		debugNetPrintf(INFO, "[ORBISGL] %s [%s][%s] Model meshes/materials: %i/%i\n",__FUNCTION__,fileName, (data->file_type == 2)? "glb" : "gltf", data->meshes_count, data->materials_count);
		// Read data buffers
		result = cgltf_load_buffers(&options, data, fileName);
		if (result != cgltf_result_success) debugNetPrintf(INFO, "[ORBISGL] %s [%s][%s] Error loading mesh/material buffers\n",__FUNCTION__,fileName, (data->file_type == 2)? "glb" : "gltf");
		...
		model.meshMaterial = malloc(model.meshCount*sizeof(int));
		for (int i = 0; i < model.meshCount; i++) model.meshes[i].vboId = (unsigned int *)calloc(MAX_MESH_VBO, sizeof(unsigned int));
			...
			for (int p = 0; p < data->meshes[i].primitives_count; p++)
			{
				for (int j = 0; j < data->meshes[i].primitives[p].attributes_count; j++)
				{
					if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_position)
					{
						...
						model.meshes[primitiveIndex].vertices = malloc(sizeof(float)*model.meshes[primitiveIndex].vertexCount*3);
					...
					{
						cgltf_accessor *acc = data->meshes[i].primitives[p].attributes[j].data;
						model.meshes[primitiveIndex].normals = malloc(sizeof(float)*acc->count*3);
					...
					else if (data->meshes[i].primitives[p].attributes[j].type == cgltf_attribute_type_texcoord)
					{
						...
						{
							model.meshes[primitiveIndex].texcoords = malloc(sizeof(float)*acc->count*2);
						...
						else
						{
							...
							debugNetPrintf(ERROR, "[ORBISGL] %s [%s] Texture coordinates must be float\n",__FUNCTION__,fileName);
						}
					}
				}
				...
				if (acc)
				{
					if (acc->component_type == cgltf_component_type_r_16u)
					{
						...
						model.meshes[primitiveIndex].indices = malloc(sizeof(unsigned short)*model.meshes[primitiveIndex].triangleCount*3);
						LOAD_ACCESSOR(unsigned short, 1, acc, model.meshes[primitiveIndex].indices)
					}
					else
					{
						// TODO: Support unsigned byte/unsigned int
						debugNetPrintf(ERROR, "[ORBISGL] %s [%s] Indices must be unsigned short\n",__FUNCTION__,fileName);
					}
				}
				else
				{
	...
	else debugNetPrintf(ERROR, "[ORBISGL] %s [%s] glTF data could not be loaded\n",__FUNCTION__,fileName);
	free(buffer);
	return model;
}
#endif
