...
#include <orbisGl2.h>
#include <orbisGl2par_shapes.h>

void par_shapes__sphere(float const* uv, float* xyz, void*);
void par_shapes__hemisphere(float const* uv, float* xyz, void*);
void par_shapes__plane(float const* uv, float* xyz, void*);
void par_shapes__klein(float const* uv, float* xyz, void*);
void par_shapes__cylinder(float const* uv, float* xyz, void*);
void par_shapes__torus(float const* uv, float* xyz, void*);
void par_shapes__trefoil(float const* uv, float* xyz, void*);

struct osn_context;
int par__simplex_noise(int64_t seed, struct osn_context** ctx);
void par__simplex_noise_free(struct osn_context* ctx);
double par__simplex_noise2(struct osn_context* ctx, double x, double y);
void par_shapes__copy3(float* result, float const* a)
{
    result[0] = a[0];
    result[1] = a[1];
    result[2] = a[2];
...
float par_shapes__dot3(float const* a, float const* b)
{
    return b[0] * a[0] + b[1] * a[1] + b[2] * a[2];
}

void par_shapes__transform3(float* p, float const* x, float const* y,
    float const* z)
{
...
void par_shapes__cross3(float* result, float const* a, float const* b)
{
...
void par_shapes__mix3(float* d, float const* a, float const* b, float t)
{
...
void par_shapes__scale3(float* result, float a)
{
    result[0] *= a;
    result[1] *= a;
    result[2] *= a;
...
void par_shapes__normalize3(float* v)
{
...
void par_shapes__subtract3(float* result, float const* a)
{
    result[0] -= a[0];
    result[1] -= a[1];
    result[2] -= a[2];
...
void par_shapes__add3(float* result, float const* a)
{
    result[0] += a[0];
    result[1] += a[1];
    result[2] += a[2];
...
float par_shapes__sqrdist3(float const* a, float const* b)
{
    float dx = a[0] - b[0];
    float dy = a[1] - b[1];
    float dz = a[2] - b[2];
...
void par_shapes__sphere(float const* uv, float* xyz, void* userdata)
{
...
void par_shapes__hemisphere(float const* uv, float* xyz, void* userdata)
{
    float phi = uv[0] * PAR_PI;
...
void par_shapes__plane(float const* uv, float* xyz, void* userdata)
{
    xyz[0] = uv[0];
    xyz[1] = uv[1];
    xyz[2] = 0;
...
void par_shapes__klein(float const* uv, float* xyz, void* userdata)
{
    float u = uv[0] * PAR_PI;
...
void par_shapes__cylinder(float const* uv, float* xyz, void* userdata)
{
    float theta = uv[1] * 2 * PAR_PI;
    xyz[0] = sinf(theta);
    xyz[1] = cosf(theta);
...
void par_shapes__torus(float const* uv, float* xyz, void* userdata)
{
    float major = 1;
...
void par_shapes__trefoil(float const* uv, float* xyz, void* userdata)
{
    float minor = *((float*) userdata);
    const float a = 0.5f;
    const float b = 0.3f;
...
par_shapes__rule* par_shapes__pick_rule(const char* name,
...
par_shapes_mesh* par_shapes__create_turtle()
{
...
par_shapes_mesh* par_shapes__apply_turtle(par_shapes_mesh* mesh,
...
void par_shapes__subdivide(par_shapes_mesh* mesh)
{
...
int par_shapes__cmp1(const void *arg0, const void *arg1)
{
...
void par_shapes__sort_points(par_shapes_mesh* mesh, int gridsize,
...
void par_shapes__weld_points(par_shapes_mesh* mesh, int gridsize,
...
int allocate_perm(struct osn_context* ctx, int nperm, int ngrad)
{
    PAR_FREE(ctx->perm);
